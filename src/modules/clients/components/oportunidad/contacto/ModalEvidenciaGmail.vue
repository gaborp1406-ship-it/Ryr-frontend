        ...
        <div v-if="visible" class="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4"
            ...
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    ...
                    <button @click="cerrarModalEmail" class="text-slate-400 hover:text-slate-600">
                ...
                <div class="px-6 py-6 space-y-4">
                    ...
                    <label
                        ...
                        <input type="file" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" class="hidden"
                            @change="onArchivoEmailSeleccionado" />
                        <template v-if="!archivoEmailPreview">
                        ...
                        <img v-else :src="archivoEmailPreview" class="max-h-48 rounded-lg object-contain" />
                    </label>
                    ...
                    <div>
                        ...
                        <textarea v-model="descripcionEmail" rows="3" placeholder="Agrega una descripcion breve..."
                ...
                <div class="px-6 py-4 border-t border-slate-100 flex justify-end gap-2">
                    ...
                    <button @click="cerrarModalEmail" :disabled="guardandoEmail"
                        class="px-4 py-2 rounded-lg text-sm font-semibold text-slate-500 hover:bg-slate-50 transition-colors disabled:opacity-40">
                        Cancelar
                    </button>

                    <p v-if="errorEmail" class="text-xs text-rose-500 -mt-2">{{ errorEmail }}</p>

                    <button @click="guardarEvidenciaEmail" :disabled="!archivoEmailPreview || guardandoEmail"
                        ...
                        <svg v-if="guardandoEmail" class="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2">
                            <path d="M21 12a9 9 0 1 1-6.219-8.56" />
                        </svg>
                        {{ guardandoEmail ? "Guardando..." : "Guardar evidencia" }}
                    </button>
...
<script src="./ModalEvidenciaGmail.ts" lang="ts"></script>

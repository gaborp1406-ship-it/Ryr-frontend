<template>
  <div class="p-6 space-y-5">
    ...
    <!-- Estado de carga -->
    <div v-if="cargando"
      class="bg-white border border-slate-200 rounded-2xl shadow-sm px-6 py-10 text-center text-sm text-slate-400">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 px-6 py-6">
        ...
        <div>
          ...
          <span
            ...
            <svg v-if="!esVideollamada" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
              class="w-3.5 h-3.5">
    ...
    <transition name="fade">
      <div v-if="modalReprogramarAbierto"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
          ...
          <!-- Contenido: formulario o animación de carga -->
          <div v-if="!guardandoReprogramacion" class="px-6 py-6">
            <p class="text-sm text-slate-400 mb-4">
              Selecciona la nueva fecha y hora para la reunión
            </p>
            ...
            <div class="flex flex-col gap-4">
              ...
              <p v-if="errorReprogramar" class="text-xs font-medium text-rose-500">
                {{ errorReprogramar }}
              </p>
            </div>
          </div>

          <!-- Animación de carga mientras se guarda -->
          ...
          <div v-if="!guardandoReprogramacion" class="px-6 py-4 border-t border-slate-100 flex justify-end gap-2">
            ...
            <button @click="confirmarReprogramar" :disabled="!nuevaFecha || !nuevaHora"
              class="px-4 py-2 rounded-lg text-sm font-semibold text-white bg-[#2d8c4a] hover:bg-[#256e3c] disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
              Confirmar
            </button>
          </div>
    ...
    <transition name="fade">
      <div v-if="modalNoAsistioAbierto" class="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
          ...
          <div class="px-6 py-6">
            ...
            <div class="flex flex-col gap-2">
              <button v-for="motivo in motivosNoAsistio" :key="motivo" @click="seleccionarMotivoNoAsistio(motivo)"
                :class="[
                ...
                <span :class="[
                  ...
                  <svg v-if="motivoNoAsistioSeleccionado === motivo" viewBox="0 0 24 24" fill="none" stroke="white"
          ...
          <div class="px-6 py-4 border-t border-slate-100 flex justify-end gap-2">
            <button @click="cerrarModalNoAsistio"
              class="px-4 py-2 rounded-lg text-sm font-semibold text-slate-500 hover:bg-slate-50 transition-colors">
              Cancelar
            </button>
            <button @click="confirmarNoAsistio" :disabled="!motivoNoAsistioSeleccionado"

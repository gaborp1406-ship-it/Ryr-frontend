  ...
  <div class="cal-page min-h-screen bg-[#f5f6f7]">
    ...
    <header
      ...
      <div class="flex flex-wrap items-center gap-4">
        <h1 class="font-barlow-cond text-2xl font-bold tracking-wide text-white">Calendario</h1>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <span class="min-w-[150px] text-center text-base font-semibold capitalize text-white">{{ headerLabel }}</span>
    ...
    <div class="flex flex-wrap items-center gap-2.5 border-b border-[#e3e3e3] bg-white px-6 py-3">
      ...
      <span v-if="loading" class="ml-auto text-sm font-semibold text-[#999]">Cargando actividades…</span>
      <span v-else-if="loadError" class="ml-auto text-sm font-semibold text-[#c0392b]">{{ loadError }}</span>
    </div>
    ...
    <main class="p-6">
      ...
      <div v-if="view === 'mes'" class="overflow-hidden rounded-xl border border-[#e3e3e3] bg-white">
        <div class="grid grid-cols-7 border-b border-[#e3e3e3] bg-[#fafafa]">
          <div v-for="d in weekDays" :key="d"
            class="px-2 py-2 text-center text-xs font-bold uppercase tracking-wide text-[#888]">{{ d }}</div>
        </div>
        <div class="grid grid-cols-7">
          <div v-for="cell in monthCells" :key="cell.dateStr"
            ...
            <div class="mb-1 flex items-center justify-between">
              <span class="flex h-5 w-5 items-center justify-center rounded-full text-sm font-semibold"
            ...
            <div class="flex flex-col gap-1">
              <div v-for="ev in cell.events.slice(0, 3)" :key="ev.id"
                class="truncate rounded px-1.5 py-[3px] text-[11px] font-medium text-white transition-transform hover:scale-[1.03]"
                :style="{ background: typeColor(ev.idTipoActividad) }" @click="openActivityDetail(ev.id, $event)">
                {{ ev.time }} · {{ ev.title }}
              </div>
              <div v-if="cell.events.length > 3" class="text-[11px] font-semibold text-[#888]">+{{ cell.events.length -
                3 }} más</div>
            </div>
          </div>
        </div>
      ...
      <div v-else-if="view === 'semana'" class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-7">
        <div v-for="col in weekCells" :key="col.dateStr"
          ...
          <div class="border-b border-[#eee] px-3 py-2.5 text-center" :class="{ 'bg-[#eef6ef]': col.isToday }">
            <div class="text-xs font-bold uppercase tracking-wide text-[#888]">{{ col.weekdayLabel }}</div>
            <div class="text-base font-bold" :class="col.isToday ? 'text-[#2d8c4a]' : 'text-black'">{{ col.day }}</div>
          </div>
          <div class="flex min-h-[260px] flex-col gap-1.5 p-2">
            <div v-for="ev in col.events" :key="ev.id"
              class="cursor-pointer rounded-md border-l-[3px] bg-[#f7f7f7] px-2 py-1.5 text-xs transition-colors hover:bg-[#eef1ee]"
              ...
              <div v-if="ev.cliente" class="text-[11px] text-[#888]">{{ ev.cliente }}</div>
            </div>
            <div v-if="col.events.length === 0" class="mt-2 text-center text-[11px] text-[#bbb]">Sin eventos</div>
      ...
      <div v-else class="mx-auto max-w-2xl overflow-hidden rounded-xl border border-[#e3e3e3] bg-white">
        <div class="border-b border-[#eee] px-5 py-3.5">
          <div class="text-base font-bold capitalize text-black">{{ dayLabel }}</div>
        </div>
        <div class="flex flex-col divide-y divide-[#f0f0f0]">
          <div v-for="h in dayHours" :key="h" class="flex gap-3 px-5 py-2.5">
            <span class="w-12 shrink-0 pt-0.5 text-xs font-semibold text-[#999]">{{ h }}</span>
            <div class="flex flex-1 flex-col gap-1.5">
              <div v-for="ev in eventsAtHour(h)" :key="ev.id"
                ...
                :style="{ background: typeColor(ev.idTipoActividad) }" @click="openActivityDetail(ev.id, $event)">
                <div class="text-[13px] font-semibold">{{ ev.time }} · {{ ev.title }}</div>
                <div v-if="ev.cliente" class="text-[11px] opacity-90">{{ ev.cliente }}</div>
              </div>
            </div>
          </div>
        </div>
...
.view-btn {
  @apply rounded-full px-3 py-1.5 text-sm font-semibold text-[#999] transition-colors;
}
...
.nav-btn {
  @apply flex h-7 w-7 items-center justify-center rounded-full border border-[#333] text-base text-white transition-colors hover:border-[#2d8c4a] hover:text-[#2d8c4a];
}

.today-btn {
  @apply rounded-full border border-[#333] px-3 py-1 text-sm font-semibold text-white transition-colors hover:border-[#2d8c4a] hover:text-[#2d8c4a];
}

.filter-select {
  @apply rounded-md border border-[#ddd] bg-white px-2.5 py-1.5 text-sm font-medium text-[#333];
}

.clear-filters-btn {
  @apply rounded-md border border-[#ddd] px-2.5 py-1.5 text-sm font-semibold text-[#888] transition-colors hover:border-[#c0392b] hover:text-[#c0392b];
}
...
.modal-header-tag {
  @apply inline-block rounded-full bg-white/20 px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-white;
}

.modal-header-title {
  @apply mt-1.5 truncate font-barlow-cond text-2xl font-bold leading-tight text-white;
}

.modal-close-btn {
  @apply flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/15 text-base text-white transition-colors hover:bg-white/30;
}
...
.modal-state-text {
  @apply text-base font-semibold text-[#888];
}
...
.modal-status-pill {
  @apply inline-flex w-fit items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-bold uppercase tracking-wide;
}
...
.modal-desc {
  @apply rounded-lg bg-[#f7f7f7] px-3.5 py-3 text-sm leading-relaxed text-[#333];
...
.modal-field-label {
  @apply text-[11px] font-bold uppercase tracking-wide text-[#999];
}

.modal-field-value {
  @apply text-sm font-semibold text-[#222];
}
...
.modal-close-footer-btn {
  @apply rounded-full bg-black px-5 py-2 text-sm font-bold text-white transition-colors hover:bg-[#2d8c4a];
}

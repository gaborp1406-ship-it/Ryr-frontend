  ...
  initMenu() {
    document.querySelectorAll('ul.menu a.menu-link').forEach((element) => {
      ...
      element.addEventListener('click', (element) => {
        //console.log(element);
        const dataHsCollapse = element.target.getAttribute('data-hs-collapse');
        const contenedor = dataHsCollapse.parentElement; // Obtener el contenedor del elemento
        const submenu = contenedor.querySelector('.sub-menu');

        if (dataHsCollapse && dataHsCollapse.includes('open')) {
          const nuevoValor = dataHsCollapse.replace('open', '').trim();
          element.target.setAttribute('data-hs-collapse', nuevoValor);
          submenu.classList.remove('hidden');
        } else {
          element.target.setAttribute('data-hs-collapse', nuevoValor + ' open');
          submenu.classList.remove('hidden');
        }
      });
    });
...
function abrirMenuItem(element) {
  var dataHsCollapse = element.getAttribute('data-hs-collapse');
  const contenedor = element.parentElement; // Obtener el contenedor del elemento
  const submenu = contenedor.querySelector('.sub-menu');

  if (dataHsCollapse && dataHsCollapse.includes('open')) {
    const nuevoValor = dataHsCollapse.replace('open', '').trim();
    element.setAttribute('data-hs-collapse', nuevoValor);
    submenu.classList.add('hidden');
  } else {
    dataHsCollapse = dataHsCollapse + ' open';
    element.setAttribute('data-hs-collapse', dataHsCollapse);
    submenu.classList.remove('hidden');
  }
}
